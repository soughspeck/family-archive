...
async function renderEventChips() {
...
// ─── Render ───────────────────────────────────────────────────────────────────
function renderTimeline(assets, isFiltered) {
    ...
    const allCheckboxes = Array.from(content.querySelectorAll('.thumb-checkbox'));
        ...
        cb.addEventListener('click', (e) => {
            ...
            if (e.shiftKey && lastCheckedIndex !== null) {
                ...
                for (let i = from; i <= to; i++) {
                    ...
                    const item = box.closest('.thumb-item');
                ...
                const item = cb.closest('.thumb-item');
                if (checking) {
                    selectedAssetIds.add(id);
                    item.classList.add('selected');
                }
    ...
    content.querySelectorAll('.thumb-check').forEach(el => {
...
async function bindOverlayTagging(asset) {
    // ── Date editing ──
    ...
    function showPeopleDropdown(query) {
        ...
        peopleDropdown.querySelectorAll('[data-person-id]').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                if (person)
                    addChip('people', person.id, person.name);
                hidePeopleEditor();
            });
        });
        peopleDropdown.querySelectorAll('[data-create-person]').forEach(btn => {
            btn.addEventListener('click', async () => {
    ...
    function showEventDropdown(query) {
        const q = query.toLowerCase().trim();
        ...
        eventDropdown.querySelectorAll('[data-event-id]').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                if (ev)
                    addChip('event', ev.id, ev.title);
                hideEventEditor();
            });
        });
        eventDropdown.querySelectorAll('[data-create-event]').forEach(btn => {
            btn.addEventListener('click', async () => {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (!target.closest('#people-tag-editor'))
            peopleDropdown.classList.add('hidden');
...
function bindChipSelect(cfg) {
    ...
    document.addEventListener('click', (e) => {
        if (!e.target.closest(`#${cfg.wrapId}`)) {
            dropdown.classList.add('hidden');
        }
    });
}
...
function updateSelectionBar() {
    ...
    document.getElementById('bulk-deselect-btn').addEventListener('click', () => {
        ...
        document.querySelectorAll('.thumb-checkbox:checked').forEach(cb => { cb.checked = false; });
        updateSelectionBar();
    });
}
async function bulkDelete() {
    ...
    await applyFilters();
    if (failed > 0)
        alert(`${failed} item${failed !== 1 ? 's' : ''} could not be deleted.`);
}
async function openBulkTagModal() {
    ...
    // ── People tagging ──
    const pSearch = document.getElementById('bulk-people-search');
    ...
    function showBulkPeople(query) {
        ...
        pDropdown.querySelectorAll('[data-pid]').forEach(btn => {
        ...
        pDropdown.querySelectorAll('[data-create-name]').forEach(btn => {
            btn.addEventListener('click', async () => {
    ...
    function renderBulkPeopleChips() {
    ...
    document.getElementById('bulk-people-chips').addEventListener('click', e => {
        ...
        if (idx !== -1)
            bulkPeopleIds.splice(idx, 1);
        renderBulkPeopleChips();
    });
    // ── Event tagging ──
    const eSearch = document.getElementById('bulk-event-search');
    ...
    function showBulkEvents(query) {
        ...
        if (!html)
            html = `<div class="tag-dropdown-empty">No events found</div>`;
        eDropdown.innerHTML = html;
        eDropdown.classList.remove('hidden');
        eDropdown.querySelectorAll('[data-eid]').forEach(btn => {
            btn.addEventListener('click', () => {
        ...
        eDropdown.querySelectorAll('[data-create-title]').forEach(btn => {
            btn.addEventListener('click', async () => {
    ...
    document.getElementById('bulk-event-chips').addEventListener('click', e => {
        ...
        if (idx !== -1)
            bulkEventIds.splice(idx, 1);
        renderBulkEventChips();
    });
    // Close dropdowns on outside click
    modal.addEventListener('click', e => {
        const t = e.target;
        if (!t.closest('#bulk-people-editor'))
            pDropdown.classList.add('hidden');
        if (!t.closest('#bulk-event-editor'))
            eDropdown.classList.add('hidden');
    });
    // ── Apply ──
    document.getElementById('bulk-tag-apply').addEventListener('click', async () => {
        if (bulkPeopleIds.length === 0 && bulkEventIds.length === 0) {
            closeBulk();
            return;
        }
        const applyBtn = document.getElementById('bulk-tag-apply');
        ...
        for (const assetId of ids) {
            ...
            const mergedEvent = bulkEventIds.length > 0
                ...
                : undefined;
            const update = {};
            if (bulkPeopleIds.length > 0)
                update.person_ids = mergedPeople.join(',');
            if (mergedEvent)
                update.event_id = mergedEvent;
