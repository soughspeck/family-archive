...
export async function openUploadModal(): Promise<void> {
...
function renderUploadStep1(): void {
...
function renderFilePreviews(): void {
  ...
  selectedFiles.forEach((f, i) => {
    ...
    const img = document.getElementById(`thumb-${i}`) as HTMLImageElement | null
    if (!img) return
...
function renderUploadStep2(): void {
  const content = document.getElementById('upload-content')!
...
async function submitUpload(): Promise<void> {
  const dateVal = (document.getElementById('meta-date') as HTMLInputElement).value.trim()
  const precision = (document.getElementById('meta-precision') as HTMLSelectElement).value
  const peopleSel = document.getElementById('meta-people') as HTMLSelectElement
  const eventVal = (document.getElementById('meta-event') as HTMLSelectElement).value
  const notes = (document.getElementById('meta-notes') as HTMLTextAreaElement).value.trim()
  ...
  for (const file of selectedFiles) {
    ...
    if (dateVal) fd.append('taken_at_manual', dateVal)
    if (precision) fd.append('date_precision_manual', precision)
    if (selectedPeople.length) fd.append('person_ids', selectedPeople.join(','))
    if (eventVal) fd.append('event_id', eventVal)
